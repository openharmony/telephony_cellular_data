...
int32_t DataSwitchSettings::SetUserDataOn(bool userDataOn)
{
    ...
    bool userDataOnTmp = IsUserDataOn();
    SetInnerUserDataOn(userDataOn);
    ...
    if (result != TELEPHONY_ERR_SUCCESS) {
        SetInnerUserDataOn(userDataOnTmp);
    }
    return result;
}

int32_t DataSwitchSettings::SetAnySimDetected(int32_t simDetected)
{
    std::shared_ptr<CellularDataSettingsRdbHelper> settingsRdbHelper = CellularDataSettingsRdbHelper::GetInstance();
    if (settingsRdbHelper == nullptr) {
        TELEPHONY_LOGE("settingsRdbHelper == nullptr!");
    ...
    int32_t result = settingsRdbHelper->PutValue(anySimDetectedUri, SIM_DETECTED_COLUMN_ENABLE, simDetected);
    TELEPHONY_LOGI("DataSwitchSettings::SetAnySimDetected result:%{public}d", result);
    return result;
}
...
int32_t DataSwitchSettings::QueryUserDataStatus(bool &dataEnabled)
{
    ...
    lastQryRet_ = settingsRdbHelper->GetValue(userDataEnableUri, CELLULAR_DATA_COLUMN_ENABLE, userDataEnable);
    if (lastQryRet_ != TELEPHONY_ERR_SUCCESS) {
...
void DataSwitchSettings::SetInnerUserDataOn(bool value)
{
    std::lock_guard<std::mutex> lock(userDataOnMutex_);
    userDataOn_ = value;
}
...
int32_t DataSwitchSettings::GetLastQryRet()
{
    return lastQryRet_;
}
} // namespace Telephony
